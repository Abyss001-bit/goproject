...
loucengInput:function(e){
 ...
 submit:function(e){
  ...
  }else if (that.data.numbercount == "") {
    wx.showModal({
      title: '提示',
      content: '编号信息不能为空',
   });
  }else{
    console.log(that.data.numbercountdigit)
      for (var i = 1; i <= that.data.numbercountdigit; ) {
        wx.setStorageSync('I', i)
        console.log(i)
        wx.request({
          // 请求地址
          url: app.globalData.URL +  '/admin/insertSeat',
          //请求方法
          method:"POST",
          //请求参数
          data:{
